...
@WebService(
		...
		wsdlLocation = "SupplierWebService.wsdl", 
...
public class SupplierPortImpl implements SupplierPortType {
	...
	public List<ProductView> searchProducts(String descText) throws BadText_Exception {
		List<ProductView> lista = listProducts();
		List<ProductView> listaFinal = new ArrayList<ProductView>();
		if(descText==null || descText.trim().equals(""))
			throwBadText("Invalid arguments!");	
		for (int i = 0; i < lista.size();i++)
			if(lista.get(i).getDesc().contains(descText))
	...
	public String buyProduct(String productId, int quantity) throws BadProductId_Exception, BadQuantity_Exception, InsufficientQuantity_Exception {
		...
		if(productId == null)
			throwBadProductId("ProductId cannot be null");
		if(productId.trim().length() == 0)
			throwBadProductId("ProductId cannot be empty");
		if(quantity <= 0)
		...
		try {
			if(supplier.productExists(productId)){
				...
				throwBadProductId("There is no product with productId: " + productId);
		}
		catch(QuantityException e){
			throwInsufficientQuantity("Desired quantity not available");
		}
